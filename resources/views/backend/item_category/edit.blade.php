...
@section('title-page', __('app.edit_item_category'))
        ...
        <div class="col-md-12">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">{{ __('app.edit_item_category') }}</h3>
                    <div class="card-tools">
                        @can('Item Category List')
                        <a href="{{ url('/item-category') }}" class="btn btn-primary"> <i class=" fas fa-list"></i>
                            {{ __('app.label_list') }} </a>
                        @endcan
                    </div>
                </div>
                ...
                <div class="card-body">
                    <form id="quickForm" action="{{ url('item-category', $item->id) }}" method="post">
                        @csrf
                        @method('patch')
                        <div class="card-body">
                            <div class="row">
