...
@section('content')
                ...
                <form action="{{ route('sales.index') }}" class="p-3 card card-primary card-outline" method="get">
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-sm-3 mb-2">
                            ...
                            <a href="{{ url('sales') }}" class="btn btn-danger"><i class="fas fa-broom"></i>
                                {{ __('app.btn_clean') }}</a>
                        </div>
                    </div>
                </form>
                    ...
                    <form action="{{ url('add-cart') }}" class="formAddCart" method="post" enctype="multipart/form-data">
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <label>{{ __('app.label_unit') }} <small class="text-red">*</small></label>
                                        <div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary btn-submit"
                                id="submit">{{ __('app.btn_accepted') }}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    {{-- modal new customer --}}
    <div class="modal fade" id="modalNewCustomer" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
        aria-hidden="false">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="{{ url('new-customer') }}" method="post">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="customer_phone" class="form-control" required
                                        value="{{ old('customer_phone') }}" placeholder="{{ __('app.phone') }}">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger"
                            data-dismiss="modal">{{ __('app.btn_close') }}</button>
                        <button type="submit" class="btn btn-primary btn-submit">{{ __('app.btn_accepted') }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalSubmitCart">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>{{ __('app.label_confirm_pay') }} <br><span
                            class="text-danger">{{ __('app.label_confirm_pay_1') }}</span> </p>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">{{ __('app.btn_no') }}</button>
    ...
    <div class="modal fade" id="staticBackdropLoading">
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <img src="{{ asset('assets/dist/img/loading.gif')}}" width="100%" alt="" srcset="">
                    <p class="text-center">{{__('app.label_loading')}}</p>
                </div>
            </div>
        </div>
    </div>
...
@section('control-sidebar')
    ...
    <div class="row p-3">
        <div class="col-sm-12" id="printarea">
            <h4 class="mb-3"> <small class="btn btn-primary" data-widget="control-sidebar"
                    data-controlsidebar-slide="true"><i class="fas fa-arrow-right"></i></small>
                {{ __('app.label_cart') }}</h4>
            <div class="card card-outline card-primary">
                <form action="{{ url('sales') }}" class="formCartList" method="post"
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <select name="customer" class="form-control rounded-0 select2 customer"
                                    id="customer">
                                    <option value="">{{ __('app.table_choose') }} {{ __('app.customer') }}
                                    </option>
                    ...
                    <div class="card-body">
                        <table id="" class="table table-bordered table-responsive-xl"
                            style="height: auto; width: 1229px; max-width: 100%;">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                @foreach ($addCart as $index => $item)
                                    <tr>
                                        ...
                                        <td>${{ $item->discount }}
                                            <input type="hidden" name="product_discount[]" class="discount"
                                                value="{{ $item->discount }}">
                                        </td>
                                        <td>${{ $item->amount }}
                                        ...
                                        <td>
                                            <button type="button" class="btn deleteAddCart"
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="deposit" class="form-control rounded-0 deposit"
                                        id="deposit" placeholder="0.0">
                                </div>
                                <div class="form-group">
                                    <label>{{ __('app.label_balance') }}</label>
                                    <input type="text" name="balance" class="form-control rounded-0 balance"
                                        id="balance" placeholder="0.0">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        {{-- <a href="{{ url('/print-add-cart') }}" target="_blink" class="btn -primary" {{ $addCart->isEmpty() ? 'disabled' : '' }}><i class="fas fa-print"></i> {{ __('app.btn_print') }}</a> --}}
                        <button type="button" class="btn btn-primary btn-pay" 
                            {{ $addCart->isEmpty() ? 'disabled' : '' }}><i class="fas fa-save"></i>
                            {{ __('app.btn_accepted') }}</button>
                        <button type="button" class="btn btn-primary confirm-customer" style="display: none"
                            data-toggle="modal" data-target="#modalConfirmCustomer"><i
