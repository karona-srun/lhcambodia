                ...
                <form action="{{ url('expend') }}" class="p-3" method="get">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <button type="submit" class="btn  btn-outline-primary exportexcel"> <i class=" fas fa-download"></i>
                                {{ __('app.btn_download') }}</button>
                            @can('Expend Create')
                                <a href="{{ url('expend/create') }}" class="btn  btn-primary"> <i
                                        class=" fas fa-plus"></i>
                                    {{ __('app.btn_add') }}</a>
                            @endcan
                        </div>
                            ...
                            @foreach ($expends->groupby('expend_option_id') as $index =>$items)
                                @foreach ($items as $row)
                                    <tr>
                                        ...
                                        @endif
                                        <td>
                                            <a href="{{ url('expend', $row->id) }}" class="btn  btn-ligth"> <i
                                                    class="fas fa-eye"></i> </a>
                                            <a href="{{ route('expend.edit', $row->id) }}" class="btn  btn-link"> <i
                                                    class="fas fa-edit"></i> </a>
                                            <button type="button" class="btn  btn-link text-danger deleteExpend"
