                ...
                <form action="{{ url('revenue') }}" class="p-3" method="get">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            @can('Revenue Create')
                                <a href="{{ url('revenue/create') }}" class="btn  btn-primary"> <i
                                        class=" fas fa-plus"></i>
                                    {{ __('app.btn_add') }}</a>
                            @endcan
                        </div>
                            ...
                            @foreach ($incomes->groupby('income_option_id') as $index => $items)
                                @foreach ($items as $row)
                                    <tr>
                                        ...
                                        <td>
                                            @can('Revenue Edit')
                                                <a href="{{ url('revenue', $row->id) }}" class="btn  btn-ligth"> <i
                                                        class="fas fa-eye"></i> </a>
                                            @endcan
                                            @can('Revenue Edit')
                                                <a href="{{ route('revenue.edit', $row->id) }}" class="btn  btn-link"> <i
                                                        class="fas fa-edit"></i> </a>
                                            @endcan
                                            @can('Revenue Delete')
                                                <button type="button" class="btn  btn-link text-danger deleteIncome"
